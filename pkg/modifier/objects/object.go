...
func getObject(objects map[interface{}]interface{}, fullKey string) (interface{}, error) {
	key, restKey, err := ParseNextKey(fullKey)
	if err != nil {
		return nil, err
	}
...
// ParseNextKey from key
// Not support array like: a[1][2]
func ParseNextKey(key string) (YamlKey, string, error) {
	if strings.HasPrefix(key, "(") {
		key = strings.TrimPrefix(key, "(")
		keys := strings.SplitN(key, ")", 2)
		if len(keys) == 1 || keys[1] == "" {
			return YamlKey{key: keys[0]}, "", nil
		}

		if strings.HasPrefix(keys[1], ".") {
			restKey := strings.TrimPrefix(keys[1], ".")
			if restKey == "" {
				return YamlKey{}, "", fmt.Errorf("parse next key error: %v", key)
			}
			return YamlKey{key: keys[0]}, restKey, nil
		} else if strings.HasPrefix(keys[1], "[") {
			restKeys := strings.SplitN(strings.TrimPrefix(keys[1], "["), "]", 2)
			if restKeys[0] == "" {
				return YamlKey{}, "", fmt.Errorf("parse next key error: %v", key)
			}
			index, err := parseIndex(restKeys[0])
			if err != nil {
				return YamlKey{}, "", fmt.Errorf("parse next key error: %v", key)
			}

			if len(restKeys) == 1 || restKeys[1] == "" {
				return YamlKey{key: keys[0], isArray: true, index: index}, "", nil
			}

			if !strings.HasPrefix(restKeys[1], ".") || restKeys[1] == "." {
				return YamlKey{}, "", fmt.Errorf("parse next key error: %v", key)
			}
			return YamlKey{key: keys[0], isArray: true, index: index}, strings.TrimPrefix(restKeys[1], "."), nil
		} else {
			return YamlKey{}, "", fmt.Errorf("parse next key error: %v", key)
		}
	} else {
		...
		if strings.Contains(keys[0], "[") {
			arrayStrings := strings.SplitN(keys[0], "[", 2)
			arrayObjectKey := arrayStrings[0]
			if !strings.HasSuffix(arrayStrings[1], "]") {
				return YamlKey{}, "", fmt.Errorf("parse next key error: %v", key)
			}
			index, err := parseIndex(strings.TrimSuffix(arrayStrings[1], "]"))
			if err != nil {
				return YamlKey{}, "", fmt.Errorf("parse next key error: %v", key)
			}
			if len(keys) == 1 {
				return YamlKey{key: arrayObjectKey, isArray: true, index: index}, "", nil
			}
			return YamlKey{key: arrayObjectKey, isArray: true, index: index}, keys[1], nil
		} else {
			if len(keys) == 1 {
				return YamlKey{key: keys[0]}, "", nil
			}
			return YamlKey{key: keys[0]}, keys[1], nil
		}
	}
}
